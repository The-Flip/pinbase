...
- **DONE — `serialize_blocking_referrer` → `BlockingReferrerSchema`.** [apps/catalog/api/soft_delete.py:68](backend/apps/catalog/api/soft_delete.py#L68) now returns the Schema directly. `soft_delete.py` took a new `from .schemas import BlockingReferrerSchema` (no cycle — `schemas.py` imports nothing from `soft_delete`). This also let the `BlockingReferrerSchema.model_validate(serialize_blocking_referrer(b))` workaround in [titles.py:1132](backend/apps/catalog/api/titles.py#L1132) collapse to a direct call. The dict-building error paths in `entity_crud._delete` / `machine_models.delete_model` / `people.delete_person` / `titles.delete_title` are unaffected: a `list[BlockingReferrerSchema]` value inside a `Status[dict[str, Any]]` body still serializes identically via Ninja.
...
- **DONE — Made the entity_crud registrars generic over `(ModelT, SchemaT)`.** Retyped both registrars with PEP 695 inline generics: `def register_entity_delete_restore[ModelT: CatalogModel, SchemaT: Schema](...)` and same for `register_entity_create`. Signatures are now `(type[ModelT], detail_qs: Callable[[], QuerySet[ModelT]], serialize_detail: Callable[[ModelT], SchemaT], response_schema: type[SchemaT], ...)`. The old `DetailQsFn = Callable[[], Any]` / `SerializeFn = Callable[[Any], Schema]` aliases are gone. Call sites were unchanged — mypy infers both vars from the explicit `model_cls=` / `response_schema=` args. The `taxonomy.py` lambda-with-default-arg pattern (`lambda cls=model_cls: _taxonomy_detail_qs(cls)`) was replaced with a nested `def detail_qs() -> QuerySet[_TaxM]` capturing `model_cls` from the enclosing `_register_*` scope — this removes the two "Cannot infer type of lambda" baseline entries the plan entry called out. `_serialize_taxonomy`'s union-of-nine parameter type remains compatible at each call site by contravariance. The `# type: ignore[misc]` on `model_cls._meta.get_field("name")` became unused once `model_cls` was a TypeVar; it was replaced with a one-line comment noting django-stubs returns `Any` for `_meta.get_field` on a TypeVar'd model, so the `isinstance` assert is the runtime narrowing. Baseline 310 → 308. Commit `105f4631`. (Note: first pass used module-level `typing.TypeVar` + `# noqa: UP047` by analogy to taxonomy's `_TaxM`; review caught that the rationale only applies to constraint lists, not simple bounds — see the "Idiom for generics over heterogeneous model classes" section for the updated distinction.)
...
- **DONE — `list_all_systems` in [systems.py](backend/apps/catalog/api/systems.py) now returns `list[SystemListSchema]`.** Swapped the dict comprehension for `SystemListSchema(...)` construction (manufacturer dict → `Ref(...)`), dropped the unused `Any` import; `cast(HasModelCount, s).model_count` stays since the annotation comes from the `Count()` annotation. No baseline entries for this file, so this is a drift cleanup rather than a baseline reduction.
- **DONE — easy-batch sweep of `list[dict[str, Any]]` / `dict[str, Any]` endpoint returns.** `list_series` → `list[SeriesListSchema]` ([series.py:119](backend/apps/catalog/api/series.py#L119)); `list_recent_models` → `list[ModelRecentSchema]` ([machine_models.py:677](backend/apps/catalog/api/machine_models.py#L677)); `get_model_edit_options` → `ModelEditOptionsSchema` ([machine_models.py:880](backend/apps/catalog/api/machine_models.py#L880), imports `EditOptionItem`); `model_delete_preview` → `ModelDeletePreviewSchema` ([machine_models.py:1022](backend/apps/catalog/api/machine_models.py#L1022)); `person_delete_preview` → `PersonDeletePreviewSchema` ([people.py:386](backend/apps/catalog/api/people.py#L386)). Dropped `Any` import from series.py only — still used elsewhere in machine_models.py / people.py. No baseline entries for these — drift cleanup.
- **Status-tuple 422 bodies still build ad-hoc dicts.** `delete_model` / `delete_person` / `delete_title` / `restore_*` / `entity_crud._delete` return `Status[dict[str, Any]]` with hand-rolled `{"detail": ..., "blocked_by": [...], ...}` bodies, even though Step 3.2 already typed the response slots as `SoftDeleteBlockedSchema | AlreadyDeletedSchema` / `PersonSoftDeleteBlockedSchema | AlreadyDeletedSchema` / `ErrorDetailSchema`. Flipping each `return Status(422, {...})` to construct the matching Schema continues the "return Schema, not dict" idiom. Structural-dispatch idiom (plan §"Idiom for Pydantic response unions") is already solved for the existing unions, so construction-time dispatch matches today's dict-time dispatch — no new design work.
- **`locations.py` traffics in nested `dict[str, Any]` trees.** `_get_location_detail`, `_ancestors_of`, `_get_manufacturers_for_pks` all return nested dicts, and the four `get_location_{1..4}` endpoints + `list_locations` return `dict[str, Any]` through to Ninja. `LocationIndexSchema` / `LocationDetailSchema` exist already. Heavier than the Step 7.1 items because the helpers are recursive-tree-shaped — plan it as its own bullet, not residual cleanup.
