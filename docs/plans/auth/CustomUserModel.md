...
2. **`email_verified` — confirmed.** Non-Optional `bool` on the SDK `User` model (`workos/types/user_management/user.py`), so present on every authenticated user. Already read at `apps/accounts/api.py:129` in the existing flow. WorkOS sets it `True` after social-OAuth login (the upstream provider has verified) and `False` for password sign-ups until the verification email is clicked. Safe to read directly off `auth_response.user` for the reactivation guard.
...
7. **Update `apps/accounts/api.py`:**
   ...
   - `get_or_create_django_user` simplifies and gains gated reactivation. The inbound payload is `auth_response.user`; for OAuth logins, derive `(provider, provider_sub)` by mapping `auth_response.authentication_method` to a provider key and — if no `SocialAccount` row already exists for `(user, provider)` — calling the new `fetch_user_identities(workos_user_id)` helper described in Spike findings. Four branches:
