...
const { submitDelete, submitUndoDelete } = await import('./model-delete');
...
describe('submitDelete', () => {
	it('returns ok with the delete response on 200', async () => {
		...
		if (out.kind === 'ok') {
			...
			expect(out.data.affected_models).toEqual(['mm-pro']);
		}
	});
	...
	it('classifies 422 with blocked_by as a blocked outcome', async () => {
		...
		if (out.kind === 'blocked') {
			...
			expect(out.blockedBy[0].relation).toBe('variant_of');
		}
	});
