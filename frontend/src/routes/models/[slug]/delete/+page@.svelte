<script lang="ts">
	...
	import type { BlockingReferrer } from './model-delete';
	import { submitDelete, submitUndoDelete } from './model-delete';
	...
	async function handleDelete() {
		...
		try {
			...
			switch (outcome.kind) {
				case 'ok': {
					const modelName = preview.model_name;
					const titleSlug = preview.title_slug;
					const changesetId = outcome.data.changeset_id;
					const handle = toast.success(`Deleted “${modelName}”.`, {
						...
						action: {
							...
							onAction: async () => {
								...
								switch (undo.kind) {
									case 'ok':
										handle.update(`Restored “${modelName}”.`, {
											dwellMs: 4_000
										});
										return;
									case 'superseded':
					...
					await goto(resolve(`/titles/${titleSlug}`));
					return;
				}
				case 'rate_limited':
...
<svelte:head>
	<title>{pageTitle(`Delete ${preview.model_name}?`)}</title>
</svelte:head>

<div class="delete-page">
	<header class="hdr">
		<h1>Delete “{preview.model_name}”?</h1>
		<p class="parent-ref">
			under
			<a href={resolveHref(`/titles/${preview.title_slug}`)}>{preview.title_name}</a>
		</p>
	</header>
	...
	{#if isBlocked}
		<section class="blocked">
			<p class="blocked-lead">
				This model can't be deleted because active records still point at it:
			</p>
			<ul>
				{#each blockedBy as ref (ref.entity_type + (ref.slug ?? ''))}
					<li>
						...
						{:else}
							{ref.name}
						{/if}
						<span class="muted">references this model via {ref.relation}</span>
					</li>
				{/each}
			</ul>
	...
	{:else}
		<section class="impact">
			...
			<p class="muted">
				You can undo this from the toast that appears on the title page, or restore the record later
	...
	{#if !isBlocked}
		<NotesAndCitationsDetails
			...
			notePlaceholder="Why are you deleting this model?"
		/>
	{/if}
