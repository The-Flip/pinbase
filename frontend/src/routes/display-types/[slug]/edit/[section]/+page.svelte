<script lang="ts">
	...
	import type { SaveMeta } from '../save-display-type-claims';
	import DisplayTypeEditorSwitch from '../DisplayTypeEditorSwitch.svelte';
	...
	$effect(() => {
		if (isMobile === true && !section) {
			goto(resolve(`/display-types/${slug}/edit/${defaultDisplayTypeSectionSegment()}`), {
				replaceState: true
			});
		}
	});
	...
	function handleCancel() {
		...
		goto(resolve(`/display-types/${slug}`));
	}
