...
class MediaVariant(TimeStampedModel):
    ...
    class Meta:
        ...
        constraints = [
            # --- string not-blank ---
            field_not_blank("role"),
            field_not_blank("storage_key"),
            field_not_blank("mime_type"),
            # --- byte_size > 0 ---
            models.CheckConstraint(
                condition=models.Q(byte_size__gt=0),
                name="media_mediavariant_byte_size_positive",
                violation_error_message="byte_size must be greater than zero.",
            ),
            # --- width/height both null or both set ---
            models.CheckConstraint(
                ...
                name="media_mediavariant_dimensions_both_or_neither",
                violation_error_message="width and height must both be null or both be set.",
                violation_error_code="cross_field",
            ),
            # --- width > 0 when set ---
            models.CheckConstraint(
                condition=models.Q(width__isnull=True) | models.Q(width__gt=0),
                name="media_mediavariant_width_positive",
                violation_error_message="width must be greater than zero.",
            ),
            # --- height > 0 when set ---
            models.CheckConstraint(
                condition=models.Q(height__isnull=True) | models.Q(height__gt=0),
                name="media_mediavariant_height_positive",
                violation_error_message="height must be greater than zero.",
            ),
            # --- storage_key must not be a full URL ---
            models.CheckConstraint(
                condition=(
                    ~models.Q(storage_key__startswith="http://")
                    & ~models.Q(storage_key__startswith="https://")
                ),
                name="media_mediavariant_storage_key_not_url",
                violation_error_message="storage_key must be a relative path, not a URL.",
            ),
            ...
            models.UniqueConstraint(
                fields=["asset", "role"],
                name="media_mediavariant_unique_asset_role",
            ),
        ]

    def __str__(self) -> str:
        return f"{self.role} for {self.asset_id}"
