...
from apps.media.models import EntityMedia, MediaAsset, MediaVariant
...
def _variant_kwargs(asset, **overrides):
    """Return valid MediaVariant kwargs with a unique storage_key."""
    defaults = {
        "asset": asset,
        "role": "original",
        "storage_key": f"media/{uuid_lib.uuid4()}/original.jpg",
        "mime_type": "image/jpeg",
        "byte_size": 5000,
        "width": 800,
        "height": 600,
...
# ---------------------------------------------------------------------------
# MediaVariant constraints
# ---------------------------------------------------------------------------


class TestMediaVariantConstraints:
    def test_valid_variant(self, asset):
        variant = MediaVariant.objects.create(**_variant_kwargs(asset))
        assert variant.pk is not None

    def test_valid_variant_without_dimensions(self, asset):
        variant = MediaVariant.objects.create(
            **_variant_kwargs(asset, width=None, height=None)
        )
        assert variant.pk is not None

    def test_blank_role_rejected(self, asset):
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(**_variant_kwargs(asset, role=""))

    def test_blank_storage_key_rejected(self, asset):
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(**_variant_kwargs(asset, storage_key=""))

    def test_blank_mime_type_rejected(self, asset):
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(**_variant_kwargs(asset, mime_type=""))

    def test_byte_size_zero_rejected(self, asset):
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(**_variant_kwargs(asset, byte_size=0))

    def test_width_without_height_rejected(self, asset):
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(
                **_variant_kwargs(asset, width=100, height=None)
            )

    def test_width_zero_rejected(self, asset):
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(**_variant_kwargs(asset, width=0, height=100))

    def test_storage_key_https_rejected(self, asset):
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(
                **_variant_kwargs(asset, storage_key="https://example.com/photo.jpg")
            )

    def test_storage_key_http_rejected(self, asset):
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(
                **_variant_kwargs(asset, storage_key="http://example.com/photo.jpg")
            )

    def test_storage_key_traversal_rejected(self, asset):
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(
    ...
    def test_duplicate_storage_key_rejected(self, user):
        """Two variants (even on different assets) cannot share a storage_key."""
        asset1 = MediaAsset.objects.create(**_asset_kwargs(user))
        asset2 = MediaAsset.objects.create(**_asset_kwargs(user))
        key = f"media/{uuid_lib.uuid4()}/shared.jpg"
        MediaVariant.objects.create(**_variant_kwargs(asset1, storage_key=key))
        with pytest.raises(IntegrityError):
            MediaVariant.objects.create(**_variant_kwargs(asset2, storage_key=key))
